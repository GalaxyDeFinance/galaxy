...
// This is the ETH/ERC20 borrow contract for Galaxy.
//
...
import {IGalaxy} from '../../interfaces/IGalaxy.sol';
import {IERC20} from '../../dependencies/openzeppelin/contracts/IERC20.sol';

contract Galaxy is IGalaxy {
    ...
    event Liquidated(string orderId, address erc20contract, address galaxyPool, uint recoveryAmount, uint reserveAmount);
   ...
   */
    ...
    function liquidatedAssets(address galaxyPool, uint256 assetAmount, uint8[] calldata vs, bytes32[] calldata rs, bytes32[] calldata ss) external {
        ...
        spendNonce = spendNonce + 1;
        // transfer tokens from this contract to the galaxyPool address
        ...
        IPool(galaxyPool).mint{value: mintValue}(address(this), assetAmount);
        if (reserveAmount > 0) {
            // If there are remaining tokens after switching to lp, transfer directly
            if (borrowInfo.tokenAddress == ETH_CONTRACT) {
                (bool success,) = galaxyPool.call{value : reserveAmount}("");
                require(success, "transfer reserve ETH fail");
            } else {
                _safeTransfer(borrowInfo.tokenAddress, galaxyPool, reserveAmount);
            }
        }
        emit Liquidated(borrowInfo.orderId, borrowInfo.tokenAddress, galaxyPool, assetAmount, reserveAmount);
    }

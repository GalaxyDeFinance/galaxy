...
import {GTHToken} from './GTHToken.sol';

/**
 * @title GalaxyPool contract
 *
 * @notice Main point of interaction with an Galaxy protocol's market
 * - Users can:
 *   # Supply
 *   # Withdraw
 *   # Borrow
 ...
 **/
contract GalaxyPool is GTHToken, IPool {
   ...
   */
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'GalaxyPool: TRANSFER_FAILED');
    }

    function _safeTransferFrom(address token, address from, address to, uint value) private {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(TRANSFER_FROM_SELECTOR, from, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'GalaxyPool: TRANSFER_FROM_FAILED');
    }
   ...
   */
    ...
    function cancelReinvest(string calldata orderId,string calldata status) external {
        uint256 size;
        ...
        INVESTMENT_EARNINGS_CONTRACT.noteCancelReinvest(orderId,status);
    }
